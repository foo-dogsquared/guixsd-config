...
(define-module (modules desktop)
  ...
  #:use-module ((modules _utils) #:prefix utils: #:select (create-pkg-interface)))
...
(utils:create-pkg-interface pkgs-browsers
  (list
    (cons 'firefox '("icecat"))
    (cons 'nyxt '("nyxt"))
    (cons 'chromium '("ungoogled-chromium"))))
...
(utils:create-pkg-interface pkgs-cad
  (list
    (cons 'default '("freecad"
		     "leocad"))
    (cons 'kicad '("kicad"
		   "kicad-symbols"
		   "kicad-packages3d"))))
...
(utils:create-pkg-interface pkgs-files
  (list
    (cons 'default '("magic-wormhole"		; Magically transfer stuff between your wormholes!
...
; Fonts? Fonts.
; TODO: Package Stix.
(utils:create-pkg-interface pkgs-fonts
  (list
    (cons 'default '("font-adobe-source-code-pro"
...
(utils:create-pkg-interface pkgs-graphics
  (list
    (cons 'default '("imagemagick"	; A image manipulation library for hipsters.
		     ))
    (cons 'raster '("gimp"		; Adobe Photoshop for libre!
		    "gimp-resynthesizer" ; A texture creation plugin for GIMP.
		    "krita"		; A good painting program.
		    ))
    (cons 'vector '("inkscape@1.0"	; Adobe Illustrator (or Affinity Designer) substitute.
		    ))
    (cons '3d '("blender"
		))))
...
(utils:create-pkg-interface pkgs-multimedia
  (list
    (cons 'default '("ffmpeg"			; Muh codecs.
...
(utils:create-pkg-interface pkgs-music
  (list
    (cons 'composition '("lilypond"		; Prevent your compositions from being lost by the time you're at your grave, engrave NOW!
    ...
    (cons 'production '("ardour"		; A DAW for hardware recording.
